...
export async function PATCH(req: NextRequest,{params}:{params:Promise<{id:string}>}) {
  try {
    ...
    if(!booking) {
      return NextResponse.json(
        {success:false,message:"booking not found to cancel"}, { status: 404 })
    }
    ...
    return NextResponse.json(
      {
        success:true,
        message:"ride confrim successfully with cash"
    },
      { status: 200 },
    );
  } catch (error) {
    console.error("Error while slecting cash method:", error);
