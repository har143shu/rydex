...
function Page() {
    ...
    if (paymentMethod == "online") {
      ...
      const paymentObject = new (window as any).Razorpay({
        ...
        handler: async (response: any) => {
          ...
          if (data.success) {
            //matlab payment verify ho gya h
            setStatus("confirmed");
            router.push(`user/ride/${bookingId}`);
          }
        },
      });
      paymentObject.open();
      ...
      const { data } = await axios.patch(`/api/payment/${bookingId}`);
      if(data.success){
         setStatus("confirmed");
          router.push(`user/ride/${bookingId}`);
      }
  ...
  const handleCancel = async () => {
    ...
    try {
      const { data } = await axios.patch(`/api/booking/${bookingId}/cancel`);
      setStatus("cancelled");
    } catch (error: any) {
      console.log(error?.response?.data?.message);
    }
      ...
      <div className="relative max-w-6xl mx-auto z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="p-8 sm:p-10">
              <div className="flex justify-between items-center mb-8">
                ...
                <div className="w-14 h-14 bg-zinc-900 rounded-2xl flex items-center shrink-0 justify-center shadow-lg">
                  <Icon size={18} className={"text-white"} />
                </div>
              </div>
              <div className="bg-zinc-50 border border-zinc-100 rounded-2xl overflow-hidden mb-8 mt-2">
          ...
          >
            ...
            <div className="flex-1 h-full p-8 sm:p-10 flex flex-col">
