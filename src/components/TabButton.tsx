...
function TabButton({active,icon,count,onClick,children}:TabButtonType) {
  return (
    <motion.div
      onClick={onClick}
      whileTap={{ scale: 0.97 }}
    ...
    >
      ...
      <span
        className={`min-w-5.5 h-5 px-1.5 text-[11px] font-bold rounded-full flex items-center justify-center transition-all
        ${
          active
            ? "bg-white text-black"
            : count > 0
