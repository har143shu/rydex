...
function ContentList({ data, type }: any) {
  ...
  const handleStartVideoKyc = async(id:any)=>{
    try{
      const {data} = await axios.patch(`/api/admin/videokyc/start/${id}`);
      ...
      {data.map((item: any, index: number) => {
        ...
        return (
          <motion.div
            ...
            className="bg-white border border-gray-100 rounded-2xl px-5 py-4 flex items-center justify-between gap-4 shadow-sm transition-shadow min-w-0"
          >
